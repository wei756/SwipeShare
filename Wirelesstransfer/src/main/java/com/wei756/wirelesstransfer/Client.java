...
public class Client extends Device {
    ...
    public Client(String address, int port, String key) {
        try {
            ...
            this.clientName = new Scanner(System.in).nextLine();

            socket = new Socket(address, port);
            System.out.println("I: " + port + "번 포트로 클라이언트 시작됨");
            ...
            System.out.println("E: 서버를 연결하는 데 실패했습니다.");
            e.printStackTrace();
        } finally {
            if (socket != null) {
                try {
     ...
     * @return
     */
    private int sendFile(String filename) {
        try {
